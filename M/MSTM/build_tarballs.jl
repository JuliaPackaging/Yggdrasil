...
if [[ "$target" == x86_64-w64-mingw32 ]]; then
...
elif [[ "$target" == i686-w64-mingw ]]; then
    cd $WORKSPACE/destdir/include
    cp $WORKSPACE/destdir/src/mpi.f90 .
    ...
    gcc -c cfg_stub.c
    gfortran -O2 -fno-range-check mpidefs-parallel.f90 mstm-intrinsics.f90 mstm-v4.0.f90 cfg_stub.o -L"${libdir}" -I"${includedir}" -lmsmpifec -lmsmpi -o "${bindir}/mstm${exeext}"
else
    cd $WORKSPACE/srcdir/MSTM/code

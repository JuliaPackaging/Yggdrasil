...
namespace MSIX
{
    namespace
    {
        ...
        void WalkDirectory(const std::string& root, WalkOptions options, Lambda visitor)
        {
            ...
            std::unique_ptr<std::remove_pointer<HANDLE>::type, decltype(&::FindClose)> find(
                FindFirstFileW(utf16Name.c_str(), &findFileData),
                &FindClose);
    ...
    std::multimap<std::uint64_t, std::string> DirectoryObject::GetFilesByLastModDate()
    {
        ...
        auto visitor = [&files, rootSize, this](
                std::string root,
                WalkOptions option,
                std::string&& name,
                std::uint64_t lastWrite) -> bool
            {
        ...
        WalkDirectory(m_root, WalkOptions::Recursive | WalkOptions::Files, visitor);
        return files;
    }
}

# SCRATCH TEST COPY — isolating whether MPI platform augmentation is the cause
# of the x86_64-apple-darwin libBlocksRuntime bug. Reverts NetCDF/NetCDFF to the
# old MPI-free pin (matching IOAPI) and removes MPI augmentation entirely.
...
# TEMP TEST: replace the broken darwin ld (missing libBlocksRuntime/libdispatch)
# with the working lld binary, since clang already links C++ fine via lld here.
# lld needs -arch/-platform_version explicitly (collect2 doesn't pass them the
# way it would for the legacy ld it thinks it's calling).
EXTRA_LD_FLAGS=""

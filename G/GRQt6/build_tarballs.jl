...
cd build
cmake $winflags -DCMAKE_INSTALL_PREFIX=$stagingprefix -DCMAKE_PREFIX_PATH=$host_prefix -DCMAKE_FIND_ROOT_PATH=$prefix -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TARGET_TOOLCHAIN} -DGR_USE_BUNDLED_LIBRARIES=ON $tifflags -DCMAKE_BUILD_TYPE=Release ..

VERBOSE=ON cmake --build . --config Release --target install -- -j${nproc}
cp -a $stagingprefix/lib/*qt6* $prefix/lib
...
if [[ $target == *"mingw32"* ]]; then
    cp -a $stagingprefix/bin/*qt6* $bindir
fi
"""

...
sources = [
   GitSource(repo, "a1e114b4c1ec997dc1c4c982cadc1975ee8db108"),
...
if [[ "${bb_full_target}" == *gpu+cuda* ]]; then
    ...
    BAZEL_BUILD_FLAGS+=(--repo_env=NVCC_PREPEND_FLAGS="-Xfatbin=-compress-all")
...
for gpu in ("none", "cuda"), mode in ("opt", "dbg"), cuda_version in ("12.8",), platform in platforms
    ...
    hermetic_cuda_version_map = Dict(
        ...
        # <https://github.com/openxla/xla/blob/main/third_party/gpus/cuda/hermetic/cuda_redist_versions.bzl>.
        "none" => "none",
        "11.8" => "11.8",
        "12.1" => "12.1.1",
        "12.3" => "12.3.1",

...
function get_component(path, work_dir, git_submodule_paths::AbstractVector{<:AbstractString}=String[])
    ...
    cd(path) do
        ...
        all_git_submodule_paths = last.(git_submodules)
        for git_submodule_path in git_submodule_paths
            if git_submodule_path ∉ all_git_submodule_paths
                @info "Skipping $git_submodule_path"
                continue
            end
        ...
        # CMake downloads
        cmake_files = String[]
        for (path, dirs, files) in walkdir(pwd())
            append!(cmake_files, [
                file_path
                for file_path in joinpath.(path, files)
                if basename(file_path) == "CMakeLists.txt" || endswith(basename(file_path), ".cmake")
            ])
        end
        
        cmake_external_project_regex = r"^[^#]*(ExternalProject_Add\((?<name>[^\s]+)[^)]+URL\s+\"?(?<url>[^\s]+)\"?[^)]+\))"ms
        cmake_external_project_urls = String[]
        for cmake_file in cmake_files
            cmake_file_content = read(cmake_file, String)
            ext_projects = collect(eachmatch(cmake_external_project_regex, cmake_file))
            for ext_project in ext_projects
                @show cmake_file, ext_project
                push!(cmake_external_project_urls, ext_project["url"])
            end
        end
        sort!(unique!(cmake_external_project_urls))

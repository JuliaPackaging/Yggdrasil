...
if [[ $target == *-w64-mingw32* ]]; then
    ...
    # disable RNAxplorer
    CONFIGURE_FLAGS="--without-rnaxplorer"
else
    # enable RNAxplorer
    CONFIGURE_FLAGS="--with-rnaxplorer --with-blas=${libdir}/libopenblas.${dlext} --with-lapack=${libdir}/liblapack.${dlext}"
    export LIBS="-lblastrampoline"
fi
...
dependencies = [
    ...
    Dependency(PackageSpec(name="LLVMOpenMP_jll", uuid="1d63c593-3942-5779-bab2-d838dc0a180e");
               platforms=filter(Sys.isbsd, platforms)),
    # we need BLAS/LAPACK for RNAxplorer, but this fails to build on windows

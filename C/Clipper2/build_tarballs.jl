...
sources = [
    ...
    # The C wrapper is hosted in Clipper.jl, next to the ccall sites that consume it,
    # and pulled in here as a second source rather than bundled in Yggdrasil.
    GitSource("https://github.com/gpeairs/Clipper.jl.git", "0d63e01ea359abaab9487252a227bbbc25e13fae"),
...
# Julia's ccall). The source is pulled from Clipper.jl above. One library, compiled
...
# expose it. A separate non-USINGZ wrapper build isn't shipped: narrow-API
# callers pay only the engine-internal 8 bytes/vertex for z they don't use,
# and adding a second library later is purely additive.
# The -fvisibility flags keep the wrapper's exports to the
...
# BSL-1.0 covers both Clipper2 and the wrapper.
# Absolute path: cwd is the wrapper directory, three levels below srcdir.

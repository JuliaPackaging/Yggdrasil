...
#
# Apple ships no static libSystem, and libc++ is an OS component with a
# stable ABI, so darwin keeps both dynamic and needs nothing here.
if [[ "${target}" == *-mingw* ]]; then
    ...
    # link-args land after -lmingwex, so repeat what libstdc++.a still needs.
    ...
    # Under the default PIC model, +crt-static emits -static-pie and needs
    # an rcrt1.o the sysroot lacks; a non-PIE link uses the ordinary crt1.o.
    RUSTFLAGS="${RUSTFLAGS} -C target-feature=+crt-static"
...
# No usable Rust toolchain shard for these targets.

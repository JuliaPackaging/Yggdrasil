...
for llvm_assertions in (false, true), llvm_full_version in llvm_full_versions
    ...
    _, _, sources, script, platforms, products, dependencies =
        configure_extraction(ARGS, llvm_full_version, "LLVM", libllvm_version;
                             assert=llvm_assertions, augmentation=true,
                             dont_dlopen=false)
    # `configure_extraction` already applies `should_build_platform` to the
    # augmented platforms, so skip this iteration if nothing is left to build.
    ...
    dependencies = map(dependencies) do dep
        ...
        # an appropriate version of LLVM via platform augmentations

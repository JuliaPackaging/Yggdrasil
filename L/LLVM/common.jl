...
if [[ -f ${LLVM_ARTIFACT_DIR}/bin/lld* ]]; then
    ...
    mv -v ${LLVM_ARTIFACT_DIR}/bin/wasm_ld* ${prefix}/tools/
else
    mv -v ${LLVM_ARTIFACT_DIR}/tools/*lld* ${prefix}/tools/
    mv -v ${LLVM_ARTIFACT_DIR}/tools/wasm_ld* ${prefix}/tools/
fi
...
rm -vrf ${prefix}/bin/{*lld*,wasm_ld*,clang*,llvm-config,mlir*}
rm -vrf ${prefix}/tools/{*lld*,wasm_ld*,clang*,llvm-config,mlir*}

...
static int
copy_info(fido_dev_info_t *di, struct udev *udev, struct udev_list_entry *udev_entry)
{
    const char *name;
    const char *path;
    char *uevent = NULL;
...
int fido_hid_manifest(fido_dev_info_t *devlist, size_t ilen, size_t *olen)
{
...
// If you only call fido_hid_manifest in the stub configuration, only this stub is needed.
...
int fido_hid_set_sigmask(void *handle, const fido_sigset_t *sigmask)
{
    struct hid_linux *ctx = handle;
    ctx->sigmask = *sigmask;
...
int fido_hid_read(void *handle, unsigned char *buf, size_t len, int ms)
{
    struct hid_linux *ctx = handle;
    ssize_t r;
...
int fido_hid_write(void *handle, const unsigned char *buf, size_t len)
{
    struct hid_linux *ctx = handle;
    ssize_t r;

...
script = bash_parse_encoded_target_triplet() * raw"""
...
linux_platforms = filter(Sys.islinux, supported_platforms(;experimental=true))
linux_platforms = unique(map(p -> begin delete!(tags(p), "libc"); p end, linux_platforms))
platforms = encode_target_platform.(linux_platforms)
...
# headers at `${compiler_prefix}/${encoded_target}/usr`.
products = Product[

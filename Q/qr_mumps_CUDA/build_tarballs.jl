...
cmake .. -DARITH="d;s;c;z" -DBUILD_SHARED_LIBS=ON -DCMAKE_INSTALL_PREFIX=$prefix -DQRM_ORDERING_AMD=ON -DQRM_ORDERING_METIS=ON \
                           -DQRM_ORDERING_SCOTCH=ON -DQRM_WITH_STARPU=ON -DQRM_WITH_CUDA=ON -DCMAKE_BUILD_TYPE=Release \
...
platforms = expand_gfortran_versions(Platform("x86_64", "linux"))
filter!(p -> libgfortran_version(p) != v"3", platforms)

# The products that we will ensure are always built
products = [
    LibraryProduct("libqrm_common", :libqrm_common, dont_dlopen=true),
    LibraryProduct("libcqrm", :libcqrm, dont_dlopen=true),
    LibraryProduct("libsqrm", :libsqrm, dont_dlopen=true),
    LibraryProduct("libdqrm", :libdqrm, dont_dlopen=true),
    LibraryProduct("libzqrm", :libzqrm, dont_dlopen=true)
]
